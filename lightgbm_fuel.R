...
# Load the data
data <- read_excel("lightgbm_fuel_data.xlsx")
data
tail(data)
...
target <- "fuel_data_mt"
exogenous <- c("crude_oil", "covid_19")
...
test_data <- data[(nrow(data)-294):nrow(data), ]  # Last 12 months for testing
test_data
tail(test_data)
...
ggplot(forecast_results, aes(x = date)) +
  ...
  labs(title = "Actual vs Predicted Fuel Data", x = "Date", y = "Fuel Data (Metric Tons)") +
...
future_data <- data.frame(
  ...
  fuel_data_mt = NA  # Add this column to match the test_data structure
)
...
# Use the trained model to forecast the fuel cost data for the next 12 months
...
ggplot(forecast_results_future, aes(x = date, y = predicted)) +
  ...
  labs(title = "Forecasted Fuel Data for the Next 12 Months", x = "Date", y = "Fuel Data (Metric Tons)") +
  theme_minimal()
...
ggplot(all_results, aes(x = date, y = value, color = type)) +
  ...
  labs(title = "Fuel Price Forecast using LightGBM Model", x = "", y = "US$ per metric tonne") +
...
path <- "C:/Users/thomas.ruaia/Documents/FFA/Data/Member request/PNG/Prediction model/data/fuel_forecast_lightgbm.xlsx"
