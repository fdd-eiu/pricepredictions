...
df <- dbGetQuery(con2, "SELECT * FROM markets.bkkmonthlyprices WHERE species = 'skj' ")
...
skj_p <- price_baht/xrate # Calculates skipjack price in USD

# Do some graphs

ts.prices <- ts(skj_p, frequency = 12, start = c(2007,1))

ts.plot(ts.prices)

brent  <- as.data.frame(data2[13:length(data2),"value"])

skj_p_ts <- ts(skj_p, frequency = 12, start = c(2007,1))
...
fitauto<- auto.arima(skj_p_ts,seasonal=TRUE)
...
plot(fc, main="Monthly skipjack forecast (Seasonal Arima)")
...
skj_p_ts_short <- ts(skj_p, frequency = 12, start = c(2007,1),end = c(2021,11))
...
plotl <- function(){
plot(y,col='red',main='Fitted model skipjack')
...
plot(fcx,main="Arimax Skipjack with US Gulf Oil and Exchange rate")

df.pred_skj <- as.data.frame(fcx)
#write.xlsx(df.pred_skj, 'skjpred.xlsx')